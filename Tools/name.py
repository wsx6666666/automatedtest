...
from Tools.random_chinese import salt
list_firstnames = ['上海']
list_midnames = [salt]
list_lastnames = ['有限公司']
def make_names():
    name = random.choice(list_firstnames)+random.choice(list_midnames)+random.choice(list_lastnames)
    return name

def appnames():
    appname = ''.join(random.sample(string.ascii_letters + string.digits, 8))
    return appname

list_company_name = ['上海测试','上海互娱','上海天美','上海天邪','上海霸下','上海无颜','上海天寒','上海传媒','上海天华','上海无憾','上海雾天']
list_company_names = ['有限公司']

def company():
    company = random.choice(list_company_name)+random.choice(list_company_names)
    return company

list_pro_name = ['我来贷','借宝贷','甜橙小贷','花啦花啦','零用金','快贷贷款','手机贷','幸福贷','福利贷','好人贷','原子贷','钱道网','快速贷','企额贷','用钱宝','91旺财','向上金服','安心投','点融','广信贷','融金宝','省呗']
list_pro_names = ['产品']
def pro():
    pro = random.choice(list_pro_name)+random.choice(list_pro_names)+random.choice(salt)
    return pro
